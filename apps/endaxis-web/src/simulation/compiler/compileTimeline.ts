...
/**
 * Returns the segment index for a given offset within an action.
 *
 * Segments are delimited by checkpoints:
 *   Segment 0: [0, checkpoints[0])
 *   Segment 1: [checkpoints[0], checkpoints[1])
 *   ...
 *   Segment N: [checkpoints[N-1], action end]
 */
export function getSegmentIndex(offset: number, checkpoints: number[]): number {
  for (let i = checkpoints.length - 1; i >= 0; i--) {
    if (offset >= checkpoints[i]) return i + 1;
  }
  return 0;
}
