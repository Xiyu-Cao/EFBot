...
type AfflictionMapKey = keyof typeof AfflictionEffectMap;
    ...
    if (hasElementalAffliction(incoming.tags)) {
      ...
      if (reaction.includes("BURST")) {
        ...
        return {
          ...
          removeIds: [],
          spawnEffects: [
            AfflictionEffectMap[reaction as AfflictionMapKey].clone(),
          ],
        };
      } else {
        // 法术异常：继承 existing 附着的层数作为异常等级
        const reactionEffect =
          AfflictionEffectMap[reaction as AfflictionMapKey].clone();
        reactionEffect.currentStacks = existingEffect.effect.currentStacks;
        return {
          name: `Arts Reaction ${existingAffliction.replace("ELEMENT_", "")}`,
          cancelIncoming: true,
          removeIds: [existingEffect.id],
          spawnEffects: [reactionEffect],
        };
      }
    }
