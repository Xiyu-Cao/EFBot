...
export function formatSimLogEntry(entry: SimLogEntry) {
  switch (entry.type) {
    ...
    case "GAUGE_CHANGE":
      return withPrefix(
        entry,
        `gauge ${entry.payload.actorId} ${entry.payload.change > 0 ? "+" : ""}${entry.payload.change.toFixed(2)} -> ${entry.payload.gauge.toFixed(2)} (${entry.payload.reason})`,
      );
    case "LEGALITY_ISSUE":
      return withPrefix(
        entry,
        `[${entry.payload.severity}] ${entry.payload.code}: ${entry.payload.message} (${entry.payload.resolution})`,
      );
  }
}
